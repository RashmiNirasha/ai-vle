        ...
        <div className="max-w-7xl mx-auto space-y-6">
                ...
                return (
                  <div
                    ...
                    className={`text-center p-2 cursor-pointer ${
                      date === new Date().getDate() ? 'bg-yellow-300' : ''
                    }`}
                  >
                    {date}
                  </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-6">
            {timetableData.map((day, index) => (
              <div
                key={index}
                className={`space-y-4 bg-white p-4 rounded-lg shadow-md hover:shadow-lg transition-shadow ${
                  currentDay === day.day.toLowerCase() ? 'border-l-4 border-blue-500' : ''
                }`}
              >
                ...
                <div className="space-y-3">
                  {day.slots.map((slot, slotIndex) => {
                    ...
                    return (
                      <div
                        key={slotIndex}
                        className={`p-4 rounded-lg ${slot.color} text-white ${
                          completedSlots[key] ? 'bg-green-500' : ''
                        }`}
                      >
